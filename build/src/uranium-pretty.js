    ...
    function Carousel(set) {
      ...
      function insertClones() {
        ...
        for (var i = 0; i < self.options.cloneLength; i++) {
          ...
          var clone = $items.eq(index).clone(true).attr("data-ur-clone", index).attr("data-ur-state", "inactive");
      ...
      this.update = function() {
        ...
        for (var i = 0; i < $items.length; i++) {
          if (self.options.fill > 0) {
            ...
            var item = $items.eq(i);
            item.width(length - parseInt(item.css("marginLeft")) - parseInt(item.css("marginRight")));
            totalWidth += length;
          }
          else {
      ...
      function moveTo(direction) {
        ...
        if (!self.options.infinite && self.options.fill > 0)
          newIndex = Math.min(self.count - self.options.fill, newIndex);
        else
          newIndex = Math.min(lastIndex, newIndex);
      ...
      // .offsetLeft/Top including margin as "part" of the element
      function offsetFront(item) {
        return item.offsetLeft - parseInt($(item).css("marginLeft"));
      }

      // offset needed to center element
      function centerOffset(item) {
        return Math.floor((viewport - $(item).outerWidth(true))/2);
      }
