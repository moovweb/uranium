...
Ur.QuickLoaders['geocode'] = (function(){
  ...
  function geocode(){
    if(navigator.geolocation){ //feature detect
      ...
      navigator.geolocation.getCurrentPosition(geoSuccess, geoError, geoDenied);  
    }
  }
  
  Geocode.prototype.setup_callbacks = function() {

    currentObj = this;
    // Set up call back for button to trigger geocoding
    if (this.elements['rg-button']) {
      x$(this.elements['rg-button']).on('click', function() {geocode();});
    } else {
...
Ur.QuickLoaders['map'] = (function(){
  ...
  Map.prototype = {
    ...
    add_coordinates: function() {
      var obj = this;
      var icon_url = x$(this.elements["icon"]).attr("url")[0];
      ...
      x$().iterate(
        ...
        function (point, index) {
          ...
          if (icon_url !== undefined) {
            icon_image = new google.maps.MarkerImage(icon_url);
          }
