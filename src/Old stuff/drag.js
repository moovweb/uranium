(function() {
	...
	var downEvent = touch ? "touchstart" : "mousedown";
	var moveEvent = touch ? "touchmove" : "mousemove";
	var upEvent = touch ? "touchend" : "mouseup";
	var cancelEvent = touch ? "touchcancel" : "mouseout";
	...
	var elementFromPoint = function( point, element ) {
		...
		mouseMove = function( point, element, last ) {
			...
			if ( last != el && el && last ) {
				...
				Syn.trigger(cancelEvent, options, last);
				options.relatedTarget = last;
				Syn.trigger("mouseover", options, el);
			}

			Syn.trigger(moveEvent, point, el || element)
			return el;
		},
		...
		startDrag = function( start, end, duration, element, callback ) {
			createEventAtPoint(downEvent, start, element);
			startMove(start, end, duration, element, function() {
				createEventAtPoint(upEvent, end, element);
				callback();
			})
		},
		center = function( el ) {
