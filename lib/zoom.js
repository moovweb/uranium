...
Ur.WindowLoaders["zoom"] = (function() {
  ...
  function Zoom(components) {
    ...
    function initialize() {
      with(self) {
        canvasWidth = canvasWidth || container.offsetWidth;
        canvasHeight = canvasHeight || container.offsetHeight;
        width = width || parseInt($img.attr("width")) || $img.getStyle("width") || img.width;
        height = height || parseInt($img.attr("height")) || $img.getStyle("height") || img.height;

        bigWidth = parseInt($img.attr("data-ur-width")) || img.naturalWidth;
        bigHeight = parseInt($img.attr("data-ur-height")) || img.naturalHeight;
        if ($img.attr("data-ur-width")[0] && $img.attr("data-ur-height")[0])
          prescale = true;

        ratio = bigWidth/width;

        boundX = (canvasWidth - bigWidth)/2;    // horizontal translation to view middle of image
        boundY = (canvasHeight - bigHeight)/2;  // vertical translation to view middle of image
      }
    }
    ...
    $img.load(function() {
      loaded_imgs.push($img.attr("data-ur-src")[0]);
