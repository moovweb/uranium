...
interactions.zoom = function ( fragment ) {
  ...
  function Zoom(set) {
    ...
    function initialize() {
      ...
      $img.data("canvasWidth", $img.data("canvasWidth") || $img.parent().outerWidth());
      $img.data("canvasHeight", $img.data("canvasHeight") || $img.parent().outerHeight());
      $img.data("width", $img.data("width") || parseInt($img.attr("width")) || parseInt($img.css("width")) || $img[0].width);
      $img.data("height", $img.data("height") || parseInt($img.attr("height")) || parseInt($img.css("height")) || $img[0].height);
      
      $img.data("bigWidth", parseInt($img.attr("data-ur-width")) || $img[0].naturalWidth);
      $img.data("bigHeight", parseInt($img.attr("data-ur-height")) || $img[0].naturalHeight);
      ...
      if (($img.attr("data-ur-width") && $img.attr("data-ur-height")) || $img.attr("src") == $img.attr("data-ur-src"))
        $img.data("prescale", true);
      
      $img.data("ratio", $img.data("bigWidth")/$img.data("width"));
      
      $img.data("boundX", ($img.data("canvasWidth") - $img.data("bigWidth"))/2);    // horizontal translation to view middle of image
      $img.data("boundY", ($img.data("canvasHeight") - $img.data("bigHeight"))/2);  // vertical translation to view middle of image
    }
    ...
    function panStart(event) {
      if (self.state == "enabled-slide") {
        ...
        if (t < 1) {
          ...
          var currentOffsetX = bound(destOffsetX + cb * slidex, [-$img.data("boundX"), $img.data("boundX")]);
          var currentOffsetY = bound(destOffsetY + cb * slidey, [-$img.data("boundY"), $img.data("boundY")]);
          transform(currentOffsetX, currentOffsetY, $img.data("ratio"));
        }
      }
    ...
    function panMove(event) {
      ...
      if (Math.abs(dx) > 5 || Math.abs(dy) > 5)
        mouseDrag = true;
      destOffsetX = bound(offsetX + dx, [-$img.data("boundX"), $img.data("boundX")]);
      destOffsetY = bound(offsetY + dy, [-$img.data("boundY"), $img.data("boundY")]);
      transform(destOffsetX, destOffsetY, $img.data("ratio"));
      dx1 = dx2;
      dy1 = dy2;
      dx2 = dx;
      dy2 = dy;
    ...
    function slide() {
      ...
      slidex = scalar * dx2;
      slidey = scalar * dy2;
      var newOffsetX = bound(destOffsetX + slidex, [-$img.data("boundX"), $img.data("boundX")]);
      var newOffsetY = bound(destOffsetY + slidey, [-$img.data("boundY"), $img.data("boundY")]);
      transform(newOffsetX, newOffsetY, $img.data("ratio"));
    ...
    function zoomHelper(x, y) {
      ...
      transform(x || 0, y || 0, $img.data("ratio"));
    }
    ...
    this.zoomIn = function(event) {
      ...
      if (!$img.data("width")) {
        ...
        $img.css("height", $img.data("height") + "px");
      }
      ...
      if (!$img.data("prescale")) {
        ...
        var translateX = bound($img.data("bigWidth")/2 - $img.data("ratio") * relX, [-$img.data("boundX"), $img.data("boundX")]);
        var translateY = bound($img.data("bigHeight")/2 - $img.data("ratio") * relY, [-$img.data("boundY"), $img.data("boundY")]);
    ...
    if ($container.attr("data-ur-touch") != "disabled") {
      ...
      $(self.img).on(downEvent + ".zoom", function(e) {
        click = down = true;
        startCoords = getEventCoords(e);
      });
      $(self.img).on(moveEvent + ".zoom", function(e) {
        var coords = getEventCoords(e);
        if (down && (Math.abs(startCoords.x - coords.x) + Math.abs(startCoords.x - coords.x)) > 0)
          click = false;
      });
      $(self.img).on("click.ur.zoom", function(e) {
        if (click) {
          setActive(this);
          if (this == $img[0])
            self.zoomIn(e);
    ...
    $img.on("load.ur.zoom", function() {
      ...
      if (!prescale && self.state == "enabled-in") {
        prescale = true;
        initialize();
    ...
    $(self.img).on("webkitTransitionEnd.ur.zoom transitionend.ur.zoom", transitionEnd);
