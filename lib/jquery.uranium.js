     ...
     function Carousel(set) {
      ...
      function moveHelper(direction) {
        ...
        if (self.options.infinite) {
          ...
          log("direction outside = " + direction);
          log("clonelength -> " + self.options.cloneLength);
          if (newIndex < self.options.cloneLength) { // at the beginning of carousel
            ...
            if(Math.abs(direction) > self.options.cloneLength) {
              var excess = (Math.abs(direction) - self.options.cloneLength)*items[1].offsetLeft;
              startingOffset += excess;
              newIndex -= Math.abs(direction) - self.options.cloneLength;
            }
            if (!self.flag.loop) {
              log("inside this loop if");
              altTransform += offset;
              log("altTransform = " + altTransform);
              translateX(altTransform);
              log("LOOP STARTING OFFSET: " + startingOffset);
              startingOffset += offset;
              log("LOOP STARTING OFFSET: " + startingOffset);
              log("LOOP OFFSET: " + offset);
            }
        ...
        var displacement = currentItem.offsetLeft - newItem.offsetLeft; // CHECK HERE?
        if (self.options.center)
