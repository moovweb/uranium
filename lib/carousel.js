...
Ur.WindowLoaders["carousel"] = (function(){
  ...
  var no_3d = navigator.userAgent.match(/Android|Firefox/);
  var transform_prefix = no_3d ? "translate(" : "translate3d(";
  var transform_suffix = no_3d ? ")" : ", 0px)";

  function translate(obj, x) {
    obj.style.webkitTransform = obj.style.MozTransform = transform_prefix + x + "px, 0px" + transform_suffix;
  }
  ...
  Carousel.prototype = {
    initialize: function() {
      ...
      // option to force translate3d on android since it seems to be faster on
      // some android devices and sometimes doesn't break anything on the page
      if (x$(this.container).attr("data-ur-translate3d")[0] == "enabled") {
        transform_prefix = "translate3d(";
        transform_suffix = ", 0px)";
      }
    ...
    adjust_spacing: function() {
      ...
      cumulative_offset += parseInt((this.snap_width - get_real_width(items[0]))/2);
    ...
    update_index: function(new_index) {
      ...
      if(this.count !== undefined) {
        if (this.multi)
          ...
          var real_index = (this.real_item_count + this.item_index - this.clone_count) % this.real_item_count + 1;
          this.count.innerHTML = real_index + " of " + this.real_item_count;
        }
        else
