...
Ur.WindowLoaders["carousel"] = (function() {
  ...
  function translate(obj, x) {
    obj.style.webkitTransform = obj.style.MozTransform = obj.style.oTransform = obj.style.transform = translate_prefix + x + "px, 0px" + translate_suffix;
  }
  ...
  Carousel.prototype = {
    ...
    adjust_spacing: function() {
      ...
      if (old_snap_width) {
        ...
        translate(this.items, this.getTranslateX(this.items) + parseInt((this.snap_width - old_snap_width)/2));
      } else
    ...
    start_swipe: function(e) {
      ...
      if (coords !== null) {
        var x_transform = this.getTranslateX(this.items);
    ...
    continue_swipe: function(e) {
      ...
      if (coords !== null) {
        ...
        if (this.infinite) {
          var items = x$(this.items).find("[data-ur-carousel-component='item']");
          var start_limit = items[this.clone_count - 1].offsetLeft; // almost at the beginning of carousel
          var end_limit = items[this.item_count - this.clone_count - 1].offsetLeft; // almost at the end of carousel
          
          if (dist >= -start_limit) {
            //this.item_index = this.item_count - this.clone_count;
            this.starting_offset -= items[this.last_index - this.clone_count].offsetLeft - items[this.clone_count - 1].offsetLeft;
            dist -= items[this.last_index - this.clone_count].offsetLeft - items[this.clone_count - 1].offsetLeft;
            this.item_index = this.item_count - this.clone_count;
          }
          else if (dist <= -end_limit) {
            this.starting_offset += items[this.last_index - this.clone_count].offsetLeft - items[this.clone_count - 1].offsetLeft;
            dist += items[this.last_index - this.clone_count].offsetLeft - items[this.clone_count - 1].offsetLeft;
            this.item_index = 0;
          }
    ...
    move_to: function(direction) {
      ...
      this.starting_offset = this.getTranslateX(this.items);
      var new_idx = this.item_index - direction;
    ...
    move_helper: function(direction) {
      ...
      if (this.infinite) {
        ...
        var old_transform = this.getTranslateX(this.items);
    ...
    momentum: function() {
      ...
      var new_transform = increment + x_transform;

      /*
      if (this.infinite) {
        var items = x$(this.items).find("[data-ur-carousel-component='item']");
        var start_limit = items[this.clone_count - 1].offsetLeft; // almost at the beginning of carousel
        var end_limit = items[this.item_count - this.clone_count].offsetLeft; // almost at the end of carousel
        
        if (new_transform >= -start_limit) { // almost at the beginning of carousel
          this.destination_offset  -= items[this.last_index - this.clone_count].offsetLeft - items[this.clone_count - 1].offsetLeft;
          new_transform -= items[this.last_index - this.clone_count].offsetLeft - items[this.clone_count - 1].offsetLeft;
        }
        //else if (new_transform >= end_limit) // almost at the end of carousel
          //new_transform -= items[this.item_count - this.clone_count].offsetLeft - items[this.clone_count].offsetLeft;
      ...
      translate(this.items, new_transform);
