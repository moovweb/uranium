...
Ur.WindowLoaders["carousel"] = (function() {
  ...
  Carousel.prototype = {
    initialize: function() {
      ...
      x$(this.button["prev"]).click(function(){self.moveTo(self.magazineCount);});
      x$(this.button["next"]).click(function(){self.moveTo(-self.magazineCount);});

      // Expose this function globally:
      this.jumpToIndex = this.jumpToIndex;

      x$(window).orientationchange(function(){self.resize();});
    ...
    readAttributes: function() {
      ...
      if (!this.options.infinite)
        cloneLength = 0;
      else if (isNaN(cloneLength) || cloneLength < this.options.fill)
        cloneLength = Math.max(1, this.options.fill);
    ...
    adjustSpacing: function() {
      ...
      if (this.options.fill > 0) {
        for (var i = this.options.fill; i > 0; i--) {
      ...
      for (var i = 0; i < items.length; i++) {
        if (this.options.fill > 0) {
          var length = divisions[i % this.options.fill];
    ...
    translate: function(x) {
      ...
      container.style.webkitTransform = container.style.MozTransform = container.style.oTransform = container.style.transform = translatePrefix + x + "px, 0px" + translateSuffix;
    }
  }
