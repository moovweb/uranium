...
Ur.WindowLoaders['carousel'] = (function(){
  ...
  var android_device = navigator.userAgent.match(/Android/);
  var transform_prefix = android_device ? "translate(" : "translate3d(";
  var transform_suffix = android_device ? ")" : ", 0px)";

  function translate(obj, x) {
    obj.style.webkitTransform = transform_prefix + x + "px, 0px" + transform_suffix;
  }
  ...
  Carousel.prototype = {
    initialize: function() {
      ...
      x$(this.items).find("[data-ur-carousel-component='item']").each(function(item, index) {
        item.setAttribute("data-ur-carousel-index", index);
      });
      ...
      x$(this.button["prev"]).on("click", (function(obj) {
        return function() {
          //obj.item_index == 0 ? obj.jump_to_index(obj.last_index) : obj.move_to(obj.magazine_count);
      ...
      x$(this.button["next"]).on("click", (function(obj) {
        return function() {
          //obj.item_index == obj.last_index ? obj.jump_to_index(0) : obj.move_to(-obj.magazine_count);
          obj.move_to(-obj.magazine_count)
        };
      })(this));
      ...
      this.infinite_enabled = !(this.container.getAttribute("data-ur-infinite") == "disabled");
      x$(this.container).attr("data-ur-infinite", this.infinite_enabled ? "enabled" : "disabled");
      
      if (this.infinite_enabled) {
        var items = x$(this.items).find("[data-ur-carousel-component='item']");
        this.clone_length = 2;
        this.clones = []; // probaby useless
        for (var i = 0; i < this.clone_length; i++) {
          var clone = items[i].cloneNode();
          this.clones.push(clone);
          x$(clone).attr("data-ur-carousel-clone", i).attr("data-ur-state", "inactive");
          clone.removeAttribute("data-ur-carousel-index");
          this.items.appendChild(clone);
        }
        
        for (var i = items.length - this.clone_length; i < items.length; i++) {
          var clone = items[i].cloneNode();
          this.clones.push(clone);
          x$(clone).attr("data-ur-carousel-clone", i).attr("data-ur-state", "inactive");
          clone.removeAttribute("data-ur-carousel-index");
        ...
        //this.item_index = this.clone_length;
      }
      
      this.adjust_spacing();
      
      this.update_index(this.infinite_enabled ? this.clone_length : 0);
      ...
      this.autoscroll_enabled = !(this.container.getAttribute("data-ur-autoscroll") == "disabled");
      x$(this.container).attr("data-ur-autoscroll", this.autoscroll_enabled ? "enabled" : "disabled");
    ...
    adjust_spacing: function() {
      ...
      if (this.infinite_enabled) {
        for (var i = 0; i < this.clone_length; i++)
          cumulative_offset -= get_real_width(items[i]);
      }
      else
        cumulative_offset -= this.snap_width*this.item_index; // initial offset
      
      cumulative_offset += (this.snap_width - get_real_width(items[0]))/2;
    ...
    autoscroll_start: function() {
      ...
      var self = this;
      self.flag.intervalId = window.setTimeout(function() {
    ...
    autoscroll_stop: function() {
      window.clearTimeout(this.flag.intervalId);
    },
    ...
    update_index: function(new_index) {
      //console.log("INSIDE UPDATE_INDEX"+new_index);
    ...
    move_to: function(direction) {
      ...
      var new_idx = this.item_index - direction;
      // KEVIN: direction is -1 if moving to right, 1 if moving to left
      this.move_helper(direction);
    },

    move_helper: function(direction) {
      //console.log("old_idx=" + this.item_index);
      ...
      //console.log("(PASUFASIOPFU)"+new_idx);
	...
	this.increment_flag = true;
      ...
      if(this.increment_flag)
      {
        ...
        if (this.infinite_enabled) {
          if (this.item_index == this.last_index - 1) {
            var infinite_offset = 0;
            for (var i = 0; i < this.clone_length; i++)
              infinite_offset -= this.clones[i].offsetWidth;
            infinite_offset += (this.snap_width - this.clones[0].offsetWidth)/2;
            translate(this.items, infinite_offset);
            this.update_index(2);
          }
        }
